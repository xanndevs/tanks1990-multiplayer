...
void Tank::draw()
{
    ...
    if (testFlag(TSF_SHIELD) && m_shield != nullptr && !testFlag(TSF_CREATE))
        m_shield->draw();
...
void Tank::update(Uint32 dt)
{
    ...
    if (testFlag(TSF_LIFE))
    {
        ...
        dest_rect.x = pos_x - (pos_x - (int)pos_x);
        dest_rect.y = pos_y - (pos_y - (int)pos_y);
        dest_rect.h = m_sprite->rect.h;
        dest_rect.w = m_sprite->rect.w;

        collision_rect.x = dest_rect.x + 1;
        collision_rect.y = dest_rect.y + 1;
...
{
    if (!(testFlag(TSF_LIFE) || testFlag(TSF_CREATE)))
        return;
    if (testFlag(TSF_ON_ICE))
    {
        new_direction = d;
        if (speed == 0.0 || m_slip_time == 0.0)
            direction = d;
    ...
    if (!stop)
    {
        ...
        case D_DOWN:
            ...
            if (pos_x - pos_x_tile < epsilon)
                pos_x = pos_x_tile;
            else if (pos_x_tile + AppConfig::tile_rect.w - pos_x < epsilon)
                pos_x = pos_x_tile + AppConfig::tile_rect.w;
            break;
        case D_RIGHT:
        case D_LEFT:
            ...
            if (pos_y - pos_y_tile < epsilon)
                pos_y = pos_y_tile;
            else if (pos_y_tile + AppConfig::tile_rect.h - pos_y < epsilon)
                pos_y = pos_y_tile + AppConfig::tile_rect.h;
            break;
        }
    }
}
