...
void Game::draw()
{
    ...
    else
    {
        ...
        if (m_pause && SDL_GetTicks() - m_pauseTime >= 300)
            renderer->drawText(nullptr, std::string("PAUSE"), {222, 40, 0, 255}, 2);

        // Reset the timer start time

        if (m_pause && SDL_GetTicks() - m_pauseTime >= 600)
...
void Game::generateEnemy()
{
    ...
    Enemy *e = new Enemy(AppConfig::enemy_starting_point.at(m_enemy_respawn_position).x, AppConfig::enemy_starting_point.at(m_enemy_respawn_position).y, type);
    m_enemy_respawn_position++;
    if (m_enemy_respawn_position >= AppConfig::enemy_starting_point.size())
        m_enemy_respawn_position = 0;
